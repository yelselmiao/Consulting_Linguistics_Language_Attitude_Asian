...
```{r age, fig.height=5, fig.width=4}
# data_agg_long %>%
...
#   facet_wrap( ~ speaker_accent, nrow = 5) +
#   theme_bw() + 
#   labs(x = 'Age', y = 'Score') + 
#   theme(axis.title.x = element_text(size = 12,face = 'bold', hjust = 0.5, vjust = -0.6),
#         axis.title.y = element_text(size = 12,face = 'bold', vjust = 3, hjust = 0.5),
...
data %>% 
  drop_na(mother_tongue) %>%  
  mutate(multi_first_lang = ifelse(str_detect(mother_tongue, ","), TRUE, FALSE)) %>% 
  group_by(multi_first_lang) %>% 
  tally()
...
data_agg_long %>%
  drop_na(mother_tongue) %>%  
  mutate(multi_first_lang = ifelse(str_detect(mother_tongue, ","), TRUE, FALSE)) %>% 
  group_by(multi_first_lang, speaker_accent) %>%
  summarise(mu = mean(score, na.rm = TRUE)) %>% 
  ggplot(aes(x=speaker_accent, y=mu,col=multi_first_lang))+
  geom_point() + 
  geom_line(aes(group = multi_first_lang)) + 
  theme_bw() + 
  labs(x = 'Accent', y = 'Mean Score') + 
  scale_colour_discrete('Speak more than one first language') + 
  theme(axis.title.x = element_text(size = 12,face = 'bold', hjust = 0.5, vjust = -0.6),
        axis.title.y = element_text(size = 12,face = 'bold', vjust = 3, hjust = 0.5),
        strip.text = element_text(size=12),
        axis.text.y = element_text(size = 10),
...
data_agg_long %>% 
  drop_na(mother_tongue) %>%  
  mutate(multi_first_lang = ifelse(str_detect(mother_tongue, ","), TRUE, FALSE)) %>% 
  ggplot(aes(y = score, x = speaker_accent, fill=multi_first_lang)) +
  geom_boxplot() + 
  theme_bw() + 
  labs(x = 'Speaker Accent', y = 'Score',fill='Speak More than One First Language') + 
  theme(axis.title.x = element_text(size = 12,face = 'bold', hjust = 0.5, vjust = -0.6),
        axis.title.y = element_text(size = 12,face = 'bold', vjust = 3, hjust = 0.5),
        strip.text = element_text(size=12),
        axis.text.y = element_text(size = 10),
...
data_agg_long %>% 
  ...
  mutate(multi_first_lang = ifelse(str_detect(mother_tongue, ","), TRUE, FALSE)) %>% 
  group_by(speaker_accent) %>% 
  wilcox_test(score ~ multi_first_lang)
```
...
```{r racial group - viz, fig.height=4, fig.width=6}
...
race_viz_2 <- data_agg_long %>%
  ...
  theme_bw() + 
  labs(x = 'Speaker Race', y = 'Score',fill='Racial Group') + 
  ggtitle("(B) Non-Asian vs. Single-race Asian") + 
  theme(axis.title.x = element_text(size = 12,face = 'bold', hjust = 0.5, vjust = -0.6),
        axis.title.y = element_text(size = 12,face = 'bold', vjust = 3, hjust = 0.5),
        strip.text = element_text(size=12),
        axis.text.y = element_text(size = 10),
...
race_viz_3 <-  data_agg_long %>%
  ...
  theme_bw() + 
  labs(x = 'Speaker Race', y = 'Score',fill='Racial Group') + 
  ggtitle("(D) Mixed-race Asian vs. Single-race Asian") + 
  theme(axis.title.x = element_text(size = 12,face = 'bold', hjust = 0.5, vjust = -0.6),
        axis.title.y = element_text(size = 12,face = 'bold', vjust = 3, hjust = 0.5),
        strip.text = element_text(size=12),
        axis.text.y = element_text(size = 10),
...
race_viz_4 <-data_agg_long %>%
  ...
  theme_bw() + 
  labs(x = 'Speaker Race', y = 'Score',fill='Racial Group') + 
  ggtitle("(C) Mixed-race Asian vs. Non-Asian") + 
  theme(axis.title.x = element_text(size = 12,face = 'bold', hjust = 0.5, vjust = -0.6),
        axis.title.y = element_text(size = 12,face = 'bold', vjust = 3, hjust = 0.5),
        strip.text = element_text(size=12),
        axis.text.y = element_text(size = 10),
...
ggarrange(race_viz_1, race_viz_2, race_viz_4, race_viz_3 , 
          hjust=-0.8,
          ncol = 2, nrow = 2)
```
...
```{r Live in French-speaing Place - Viz}
data_agg_long %>%
  drop_na(french_speaking_place_cate) %>% 
  ggplot(aes(y = score, x = speaker_race, fill=french_speaking_place_cate)) +
  geom_boxplot() + 
  theme_bw() + 
  labs(x = 'Speaker Race', y = 'Score',fill='Live in French-Speaking Place') + 
  theme(axis.title.x = element_text(size = 12,face = 'bold', hjust = 0.5, vjust = -0.6),
        axis.title.y = element_text(size = 12,face = 'bold', vjust = 3, hjust = 0.5),
        strip.text = element_text(size=12),
        axis.text.y = element_text(size = 10),
...
data_agg_long %>%
  ...
  ggplot(aes(y = score, x = speaker_race, fill=pref_french)) +
  geom_boxplot() + 
  theme_bw() + 
  labs(x = 'Speaker Race', y = 'Score',fill='Preferred Variety of French') + 
  theme(axis.title.x = element_text(size = 12,face = 'bold', hjust = 0.5, vjust = -0.6),
        axis.title.y = element_text(size = 12,face = 'bold', vjust = 3, hjust = 0.5),
        strip.text = element_text(size=12),
        axis.text.y = element_text(size = 10),
...
```{r Teacher Origin - Viz}
data_agg_long %>%
  drop_na(teacher_origin) %>%
  ggplot(aes(y = score, x = speaker_race, fill=teacher_origin)) +
  geom_boxplot() + 
  theme_bw() + 
  labs(x = 'Speaker Race', y = 'Score',fill='Origin of French Teacher') + 
  scale_fill_discrete(name = "Origin of French Teacher", labels = c('French', 'Other', 'Quebec')) + 
  theme(axis.title.x = element_text(size = 12,face = 'bold', hjust = 0.5, vjust = -0.6),
        axis.title.y = element_text(size = 12,face = 'bold', vjust = 3, hjust = 0.5),
        strip.text = element_text(size=12),
        axis.text.y = element_text(size = 10),
...
```{r French Speaking Place/Preference/Teacher Origin - test}
# KW 
data_agg_long %>%
  filter(speaker_race == 'asian') %>%
  kruskal_test(score ~ french_speaking_place_cate)
...
data_agg_long %>%
  filter(speaker_race == 'asian') %>%
  drop_na(teacher_origin) %>%
  dunn_test(score ~ teacher_origin)
```
...
data_agg_long %>%
  drop_na(mother_tongue) %>%  
  mutate(multi_first_lang = ifelse(str_detect(mother_tongue, ","), TRUE, FALSE)) %>% 
  ggplot(aes(y = score, x = speaker_race, fill=multi_first_lang)) +
  geom_boxplot() + 
  theme_bw() + 
  labs(x = 'Speaker Race', y = 'Score',fill='Speak more than one first language') + 
  theme(axis.title.x = element_text(size = 12,face = 'bold', hjust = 0.5, vjust = -0.6),
        axis.title.y = element_text(size = 12,face = 'bold', vjust = 3, hjust = 0.5),
        strip.text = element_text(size=12),
        axis.text.y = element_text(size = 10),
...
data_agg_long %>%
  ...
  mutate(multi_first_lang = ifelse(str_detect(mother_tongue, ","), TRUE, FALSE)) %>% 
   filter(speaker_race == 'asian') %>%
  kruskal_test(score ~ multi_first_lang)
...
data_agg_long %>%  
  filter(speaker_race == 'white') %>%
  mutate(continent_cate = ifelse(continent %in% c('Africa', 'Europe', 'America'), 'Africa, Europe, America', continent))  %>%
  dunn_test(score ~ continent_cate)  
...
## Correlation analysis for numberic variables 

```{r Year of Speaking French}
data_agg_long %>%
  mutate(year_of_french = as.numeric(year_of_french)) %>% 
...
```{r Months of living in French-speaking places}
data_agg_long %>%  
  mutate(month_live_in_fr_cate = case_when(month_live_in_fr_env %in% c('1 - 3', '4 - 6', '7 - 12')~'1 - 12',
                                           month_live_in_fr_env == '> 12' ~ '> 12', 
                                           month_live_in_fr_env == '< 1' ~ '< 1')) %>% 
...
data_agg_long %>%
  ...
  geom_jitter() +
  geom_smooth() +
  facet_wrap( ~ speaker_race, nrow = 5) +
  theme_bw() +
  labs(x = 'Age', y = 'Score') +
  theme(axis.title.x = element_text(size = 12,face = 'bold', hjust = 0.5, vjust = -0.6),
        axis.title.y = element_text(size = 12,face = 'bold', vjust = 3, hjust = 0.5),
...
data_agg_long %>%
  drop_na(age) %>% 
  group_by(speaker_race) %>% 
  summarise(pearson_cor = round(cor.test(age, score, method = 'pearson', exact = FALSE)$estimate,3),
            pearson_p = round(cor.test(age, score, method = 'pearson', exact = FALSE)$p.value,3),
            )
...
data_agg_long %>%
  drop_na(mother_tongue) %>%  
  mutate(multi_first_lang = ifelse(str_detect(mother_tongue, ","), TRUE, FALSE)) %>% 
  group_by(speaker_race, multi_first_lang) %>% 
  summarise(mu = mean(score, na.rm = TRUE))
```
