...
colnames(res_tb_und) <- c('mean_Acadian', 'mean_African', 'mean_l2','mean_French', 'mean_Quebec', 'F', 'p-value')
...
recording_df_agg_long <- recording_df_agg %>%  
  ...
  mutate(speaker_race = ifelse(rec_index %in% c('rec_1', 'rec_4', 'rec_7', 'rec_8', 'rec_9'), 'white', 'asian'), 
         speaker_accent = case_when(rec_index %in% c('rec_1', 'rec_6') ~ 'Quebec', 
                                    rec_index %in% c('rec_2', 'rec_9') ~ 'French',
...
# one-sided t-test
t.test(w_df$score, a_df$score, alternative= "greater", paired = TRUE) 

# one-sided Wilcoxon test
wilcox.test(w_df$score, a_df$score, alternative= "greater", paired = TRUE)
```
